...
-----
    ...
    python main_pipeline.py ingest --file cv_context.md

    # for every new application target
    python main_pipeline.py generate --url "https://example-lab.edu/careers" \
...
def load_vectordb(persist_dir: str = PERSIST_DIR) -> Chroma:
    if not Path(persist_dir).exists():
        raise RuntimeError(
            "No vector store found. Run `python main_pipeline.py ingest --file <cv.md>` first."
        )
    return Chroma(
        collection_name=COLLECTION_NAME,
...
def extract_org_name(url: str, target_chunks: list[Document]) -> str:
    """
    Best-effort extraction of a human-readable organization name from the
    first target chunk's text (usually the page title / h1 after WebBaseLoader
    strips HTML). Falls back to the domain if no short line is found.

    Updated to accept list[Document] instead of a raw string so it works
    with the new scrape_and_chunk_target_page return type.
    """
    import urllib.parse

    # Use the first non-empty, reasonably short line of the first chunk.
    if target_chunks:
        for line in target_chunks[0].page_content.splitlines():
            line = line.strip()
...
def generate_email(url: str, role: str, persist_dir: str = PERSIST_DIR) -> str:
    ...
    if not contact_file.exists():
        raise RuntimeError(
            "No contact_info.json found. Re-run `python main_pipeline.py ingest --file <cv.md>` "
