...
def extract_org_name(url: str, scraped_text: str) -> str:
    """
    Best-effort extraction of a human-readable organization name from the
    scraped page text. Tries the first non-empty line (often a <title> or <h1>
    equivalent after WebBaseLoader strips HTML), then falls back to the domain.
    """
    import urllib.parse

    # First non-empty line of scraped text is usually the page title / h1
